"""后端 i18n 翻译模块 — 由 scripts/generate_i18n.py 自动生成，勿手动编辑"""

TRANSLATIONS = {
    'zh-CN': {
        'dineIn': '堂食',
        'err_admin_cannot_delete': '管理员账户无法注销。如需停用，请联系其他管理员操作。',
        'err_already_settled': '这笔进货已经清账了',
        'err_amount_invalid': '金额格式无效',
        'err_amount_positive': '金额不能为0',
        'err_bill_date_format': '账单日期格式必须为 YYYY-MM-DD',
        'err_code_expired': '验证码已过期，请重新注册',
        'err_code_send_failed': '验证码发送失败，请稍后再试',
        'err_date_format': '日期格式必须为 YYYY-MM-DD',
        'err_email_404': '未找到该邮箱的注册记录',
        'err_email_code_expired': '验证码已过期',
        'err_email_invalid': '邮箱格式不正确',
        'err_email_not_found': '未找到该邮箱的注册记录',
        'err_email_registered': '该邮箱已被注册',
        'err_email_required': '请输入邮箱',
        'err_email_same': '新邮箱与当前邮箱相同',
        'err_empty_email_code': '请输入邮箱和验证码',
        'err_empty_fields': '用户名和密码不能为空',
        'err_field_negative': '{field} 不能为负',
        'err_field_not_number': '{field} 必须是有效数字',
        'err_field_too_large': '{field} 数值超出合理范围',
        'err_fill_all_fields': '请填写所有字段',
        'err_incomplete': '请填写完整信息',
        'err_invalid_account': '无效的支付方式',
        'err_invalid_category': '无效的分类',
        'err_invalid_reconciled_by': '录入人格式无效',
        'err_invalid_type': 'type 只能是 income 或 expense',
        'err_invoice_file_too_large': '文件大小不能超过 10MB',
        'err_invoice_file_type': '仅支持 PDF / JPG / PNG 文件',
        'err_invoice_not_found': '开票记录不存在',
        'err_missing_date': '缺少日期',
        'err_missing_fields': '缺少必填字段: {fields}',
        'err_need_verify': '邮箱未验证，请先验证',
        'err_new_email_required': '请输入新邮箱',
        'err_not_found': '记录不存在',
        'err_partner_cannot_delete': '合伙人账户无法注销。如需停用，请联系管理员操作。',
        'err_pw_requirements': '密码须8位以上，包含字母、数字、特殊字符',
        'err_recon_confirm': '需要 confirm="YES" 二次确认',
        'err_resend_failed': '验证码发送失败',
        'err_reset_code_expired': '验证码已过期，请重新获取',
        'err_reset_code_wrong': '验证码错误',
        'err_reset_same_password': '新密码不能与旧密码相同',
        'err_review_pending': '账户待管理员审核，暂不可登录',
        'err_session_expired': '登录已过期，请重新登录',
        'err_session_kicked': '您的账号在其他设备登录，当前会话已退出',
        'err_password_changed': '密码已修改，请重新登录',
        'err_email_changed': '邮箱已修改，请重新登录',
        'err_too_many_attempts': '尝试次数过多，请等待 {mins} 分 {secs} 秒后再试',
        'err_user_linked_partner': '该用户身为合伙人，不可删除，解除关联，即可删除。',
        'err_username_exists': '用户名已存在',
        'err_username_invalid': '用户名仅支持字母、数字、下划线和中文，2-32位',
        'err_wrong_code': '验证码错误',
        'err_wrong_credentials': '用户名或密码错误',
        'flashSale': '闪购',
        'goods': '采购',
        'jd': '京东',
        'meituan': '美团',
        'meituanCashier': '美团收银',
        'meituanTuan': '美团团购',
        'meituanWaimai': '美团外卖',
        'msg_code_resent': '验证码已重新发送',
        'msg_code_sent': '验证码已发送至 {email}，请查收',
        'msg_email_changed': '邮箱修改成功',
        'msg_email_code_sent': '验证码已发送',
        'msg_forgot_sent': '如该邮箱已注册，验证码已发送',
        'msg_recon_cleared': '已清空所有对账记录',
        'msg_reset_ok': '密码重置成功！请使用新密码登录',
        'msg_verify_ok': '邮箱验证成功！请登录',
        'msg_webauthn_bound': '面容登录已开启',
        'msg_webauthn_unbound': '面容登录已关闭',
        'payAlipay': '支付宝',
        'payCash': '现金',
        'payWechat': '微信',
        'pdfColName': '商品名称',
        'pdfColQty': '数量',
        'pdfColSpec': '规格',
        'pdfColSubtotal': '小计',
        'pdfColUnitPrice': '单价',
        'pdfGenDate': '生成日期',
        'pdfImgLabel': '采购凭证',
        'pdfLabelCategory': '类别',
        'pdfLabelDate': '进货日期',
        'pdfLabelPayment': '支付',
        'pdfOperator': '经办人',
        'pdfSubtitle': '采购订单 / 收货凭证',
        'pdfTitle': '进货单',
        'pdfTotalCNY': '合计（人民币）',
        'procBatchLabel': '采购批次',
        'procNoteOptional': '可选备注',
        'procNowBatch': '第{n}次进货',
        'shangouWaimai': '闪购外卖',
        'tuan': '团购',
    },
    'zh-TW': {
        'dineIn': '堂食',
        'err_admin_cannot_delete': '管理員帳戶無法註銷。如需停用，請聯繫其他管理員操作。',
        'err_already_settled': '這筆進貨已經清賬了',
        'err_amount_invalid': '金額格式無效',
        'err_amount_positive': '金額不能為0',
        'err_bill_date_format': '賬單日期格式必須為 YYYY-MM-DD',
        'err_code_expired': '驗證碼已過期，請重新註冊',
        'err_code_send_failed': '驗證碼發送失敗，請稍後再試',
        'err_date_format': '日期格式必須為 YYYY-MM-DD',
        'err_email_404': '未找到該郵箱的註冊記錄',
        'err_email_code_expired': '驗證碼已過期',
        'err_email_invalid': '郵箱格式不正確',
        'err_email_not_found': '未找到該郵箱的註冊記錄',
        'err_email_registered': '該郵箱已被註冊',
        'err_email_required': '請輸入郵箱',
        'err_email_same': '新郵箱與當前郵箱相同',
        'err_empty_email_code': '請輸入郵箱和驗證碼',
        'err_empty_fields': '使用者名稱和密碼不能為空',
        'err_field_negative': '{field} 不能為負',
        'err_field_not_number': '{field} 必須是有效數字',
        'err_field_too_large': '{field} 數值超出合理範圍',
        'err_fill_all_fields': '請填寫所有欄位',
        'err_incomplete': '請填寫完整資訊',
        'err_invalid_account': '無效的支付方式',
        'err_invalid_category': '無效的分類',
        'err_invalid_reconciled_by': '錄入人格式無效',
        'err_invalid_type': 'type 只能是 income 或 expense',
        'err_invoice_file_too_large': '檔案大小不能超過 10MB',
        'err_invoice_file_type': '僅支援 PDF / JPG / PNG 檔案',
        'err_invoice_not_found': '開票記錄不存在',
        'err_missing_date': '缺少日期',
        'err_missing_fields': '缺少必填欄位: {fields}',
        'err_need_verify': '郵箱未驗證，請先驗證',
        'err_new_email_required': '請輸入新郵箱',
        'err_not_found': '記錄不存在',
        'err_partner_cannot_delete': '合夥人帳戶無法註銷。如需停用，請聯繫管理員操作。',
        'err_pw_requirements': '密碼須8位以上，包含字母、數字、特殊字符',
        'err_recon_confirm': '需要 confirm="YES" 二次確認',
        'err_resend_failed': '驗證碼發送失敗',
        'err_reset_code_expired': '驗證碼已過期，請重新獲取',
        'err_reset_code_wrong': '驗證碼錯誤',
        'err_reset_same_password': '新密碼不能與舊密碼相同',
        'err_review_pending': '帳戶待管理員審核，暫不可登入',
        'err_session_expired': '登錄已過期，請重新登錄',
        'err_session_kicked': '您的帳號在其他裝置登入，當前工作階段已退出',
        'err_password_changed': '密碼已修改，請重新登入',
        'err_email_changed': '電子郵件已修改，請重新登入',
        'err_too_many_attempts': '嘗試次數過多，請等待 {mins} 分 {secs} 秒後再試',
        'err_user_linked_partner': '該用戶身為合夥人，不可刪除，解除關聯，即可刪除。',
        'err_username_exists': '使用者名稱已存在',
        'err_username_invalid': '使用者名稱僅支援字母、數字、底線和中文，2-32位',
        'err_wrong_code': '驗證碼錯誤',
        'err_wrong_credentials': '使用者名稱或密碼錯誤',
        'flashSale': '閃購',
        'goods': '採購',
        'jd': '京東',
        'meituan': '美團',
        'meituanCashier': '美團收銀',
        'meituanTuan': '美團團購',
        'meituanWaimai': '美團外賣',
        'msg_code_resent': '驗證碼已重新發送',
        'msg_code_sent': '驗證碼已發送至 {email}，請查收',
        'msg_email_changed': '郵箱修改成功',
        'msg_email_code_sent': '驗證碼已發送',
        'msg_forgot_sent': '如該郵箱已註冊，驗證碼已發送',
        'msg_recon_cleared': '已清空所有對賬記錄',
        'msg_reset_ok': '密碼重設成功！請使用新密碼登入',
        'msg_verify_ok': '郵箱驗證成功！請登入',
        'msg_webauthn_bound': '面容登錄已開啟',
        'msg_webauthn_unbound': '面容登錄已關閉',
        'payAlipay': '支付寶',
        'payCash': '現金',
        'payWechat': '微信',
        'pdfColName': '商品名稱',
        'pdfColQty': '數量',
        'pdfColSpec': '規格',
        'pdfColSubtotal': '小計',
        'pdfColUnitPrice': '單價',
        'pdfGenDate': '生成日期',
        'pdfImgLabel': '採購憑證',
        'pdfLabelCategory': '類別',
        'pdfLabelDate': '進貨日期',
        'pdfLabelPayment': '支付',
        'pdfOperator': '經辦人',
        'pdfSubtitle': '採購訂單 / 收貨憑證',
        'pdfTitle': '進貨單',
        'pdfTotalCNY': '合計（人民幣）',
        'procBatchLabel': '採購批次',
        'procNoteOptional': '可選備註',
        'procNowBatch': '第{n}次進貨',
        'shangouWaimai': '閃購外賣',
        'tuan': '團購',
    },
    'en': {
        'dineIn': 'Dine-in',
        'err_admin_cannot_delete': 'Admin account cannot be deleted. Please contact another admin if you need to disable it.',
        'err_already_settled': 'This batch is already settled',
        'err_amount_invalid': 'Invalid amount format',
        'err_amount_positive': 'Amount cannot be 0',
        'err_bill_date_format': 'Bill date format must be YYYY-MM-DD',
        'err_code_expired': 'Verification code expired, please register again',
        'err_code_send_failed': 'Failed to send verification code, please try again',
        'err_date_format': 'Date format must be YYYY-MM-DD',
        'err_email_404': 'No registration found for this email',
        'err_email_code_expired': 'Verification code expired',
        'err_email_invalid': 'Invalid email format',
        'err_email_not_found': 'No registration found for this email',
        'err_email_registered': 'This email is already registered',
        'err_email_required': 'Email is required',
        'err_email_same': 'New email is the same as current email',
        'err_empty_email_code': 'Email and verification code required',
        'err_empty_fields': 'Username and password required',
        'err_field_negative': '{field} cannot be negative',
        'err_field_not_number': '{field} must be a valid number',
        'err_field_too_large': '{field} value out of reasonable range',
        'err_fill_all_fields': 'Please fill in all fields',
        'err_incomplete': 'Please fill in all fields',
        'err_invalid_account': 'Invalid payment method',
        'err_invalid_category': 'Invalid category',
        'err_invalid_reconciled_by': 'Invalid reconciled_by format',
        'err_invalid_type': 'type must be income or expense',
        'err_invoice_file_too_large': 'File size cannot exceed 10MB',
        'err_invoice_file_type': 'Only PDF / JPG / PNG files are supported',
        'err_invoice_not_found': 'Invoice record not found',
        'err_missing_date': 'Missing date',
        'err_missing_fields': 'Missing required fields: {fields}',
        'err_need_verify': 'Email not verified, please verify first',
        'err_new_email_required': 'Please enter new email',
        'err_not_found': 'Record not found',
        'err_partner_cannot_delete': 'Partner accounts cannot be deleted. Please contact an admin to deactivate.',
        'err_pw_requirements': 'Password must be 8+ chars with letters, digits, and special characters',
        'err_recon_confirm': 'Requires confirm="YES"',
        'err_resend_failed': 'Failed to send verification code',
        'err_reset_code_expired': 'Verification code expired, please request a new one',
        'err_reset_code_wrong': 'Incorrect verification code',
        'err_reset_same_password': 'New password cannot be the same as the old password',
        'err_review_pending': 'Account pending admin review',
        'err_session_expired': 'Session expired, please login again',
        'err_session_kicked': 'Signed in elsewhere. This session was ended.',
        'err_password_changed': 'Password changed, please login again',
        'err_email_changed': 'Email changed, please login again',
        'err_too_many_attempts': 'Too many attempts, please wait {mins}m {secs}s',
        'err_user_linked_partner': 'This user is linked as a partner and cannot be deleted. Unlink the partner first to delete.',
        'err_username_exists': 'Username already taken',
        'err_username_invalid': 'Username must be 2-32 characters: letters, digits, underscores, or Chinese',
        'err_wrong_code': 'Incorrect verification code',
        'err_wrong_credentials': 'Incorrect username or password',
        'flashSale': 'Flash Sale',
        'goods': 'Purchase',
        'jd': 'JD.com',
        'meituan': 'Meituan',
        'meituanCashier': 'Meituan POS',
        'meituanTuan': 'Meituan Tuan',
        'meituanWaimai': 'Meituan Waimai',
        'msg_code_resent': 'Verification code resent',
        'msg_code_sent': 'Verification code sent to {email}, please check',
        'msg_email_changed': 'Email changed successfully',
        'msg_email_code_sent': 'Verification code sent',
        'msg_forgot_sent': 'If registered, a verification code has been sent',
        'msg_recon_cleared': 'All reconciliation records cleared',
        'msg_reset_ok': 'Password reset! Please login with your new password',
        'msg_verify_ok': 'Email verified! Please login',
        'msg_webauthn_bound': 'Face ID login enabled',
        'msg_webauthn_unbound': 'Face ID login disabled',
        'payAlipay': 'Alipay',
        'payCash': 'Cash',
        'payWechat': 'WeChat',
        'pdfColName': 'Product',
        'pdfColQty': 'Qty',
        'pdfColSpec': 'Spec',
        'pdfColSubtotal': 'Subtotal',
        'pdfColUnitPrice': 'Unit Price',
        'pdfGenDate': 'Generated',
        'pdfImgLabel': 'Receipts',
        'pdfLabelCategory': 'Category',
        'pdfLabelDate': 'Purchase Date',
        'pdfLabelPayment': 'Payment',
        'pdfOperator': 'Operator',
        'pdfSubtitle': 'PURCHASE ORDER / RECEIPT',
        'pdfTitle': 'Purchase Order',
        'pdfTotalCNY': 'Total (CNY)',
        'procBatchLabel': 'Batch',
        'procNoteOptional': 'Note (optional)',
        'procNowBatch': 'Batch #{n}',
        'shangouWaimai': 'Flash Waimai',
        'tuan': 'Group Buy',
    },
}


def get_lang(request):
    """从请求头获取语言，优先级：X-Lang > Accept-Language > 默认 zh-CN"""
    x_lang = request.headers.get('X-Lang', '')
    if x_lang in TRANSLATIONS:
        return x_lang
    best = request.accept_languages.best_match(['zh-CN', 'zh-TW', 'en'], default='zh-CN')
    return best


def t(key, lang='zh-CN', **kwargs):
    """翻译单个 key"""
    msg = TRANSLATIONS.get(lang, TRANSLATIONS['zh-CN']).get(key, key)
    if kwargs:
        msg = msg.format(**kwargs)
    return msg
