    ...
    with get_db() as db:
        for rec in data.get('daily_revenue', []):
            try:
                ...
                db.execute(
                    'INSERT OR REPLACE INTO daily_revenue (date, revenue, turnover, jd_revenue, note, user_id, archived) VALUES (?,?,?,?,?,?,?)',
                    (rec['date'], rec['revenue'], rec['turnover'], rec.get('jd_revenue', 0), rec.get('note', ''), user_id, archived)
                )
